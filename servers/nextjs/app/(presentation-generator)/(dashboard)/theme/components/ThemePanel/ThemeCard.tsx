...
export const ThemeCard: React.FC<ThemeCardProps> = ({ theme, onSelect, onDelete, showDeleteButton = true }) => {
  ...
  return (<div
    className={` group rounded-xl border w-[305px] cursor-pointer transition-all relative bg-card border-[#EDEEEF]   hover:shadow-md`}
    onClick={() => onSelect(theme)}

  >
    {showDeleteButton && <button
      className="absolute hidden group-hover:block duration-300 transition-all -top-3 -right-3 z-10 bg-card rounded-full p-2  border border-[#EDEEEF] hover:bg-muted hover:text-foreground"
        ...
        >
          ...
          <div className="flex border-t border-border">
            <button
              ...
              className="flex-1 px-4 py-3.5 text-sm font-medium text-foreground hover:bg-muted transition-colors"
            >
              Cancel
            </button>
            <button
    ...
    <div className='relative h-[250px] flex justify-center items-center '>
      ...
      <div className=" absolute top-0 left-0 flex items-center justify-between gap-2  z-[2] p-2">
        <ToolTip content='Font' >
          ...
          <p className=" text-xs font-syne  flex gap-1 capitalize  items-center  rounded-[100px]  px-2.5 py-1 bg-[#3A3A3AF5] text-foreground font-semibold  z-40 ">
        ...
        {theme.company_name && <ToolTip content='COMPANY'>
          ...
          <p className=" text-xs font-syne  flex gap-1 capitalize  items-center  rounded-[100px]  px-2.5 py-1 bg-[#3A3A3AF5] text-foreground font-semibold  text-ellipsis overflow-hidden whitespace-nowrap z-40 ">
    ...
    <div className='px-5 border-t rounded-b-xl border-[#EDEEEF] w-full py-2.5 h-[80px] bg-card flex items-center justify-between'>
      <div>
